 ...
 */
// 某用户新增某天一个专注
export const addNewFocusApiByDate = (postData)=>{
    const postUrl = baseApi + '/focus/addFocus'
...
//某用户获取某天的专注
export const getFocusByDateApi = (e)=>{
    const getUrl = baseApi + '/focus/getAllFocus'
    return axios.get(getUrl, {
        params:{
            userName: e.userName,
            date: e.date
...
//删除某专注
export const deleteFocusApi = (focusId)=>{
    const getUrl = baseApi + '/focus/deleteFocus'
    return axios.get(getUrl,{
        params:{
            focusId: focusId
        }
...
//更新某专注
export const updateFocusApi = (postData)=>{
    const postUrl = baseApi + '/focus/updateFocus'
    return axios.post(postUrl, postData,{
        headers: {
            'Content-Type': 'application/json' // 设置请求头
        }
